...
% Do not edit by hand!
\RCS$Revision: 132856 $
\RCS$HeadURL: svn+ssh://svn.cern.ch/reps/tdr2/notes/HIG-12-021/tags/ICHEP3p5invfb/HIG-12-021.tex $
\RCS$Id: HIG-12-021.tex 132856 2012-06-26 22:41:55Z kalanand $
%%%%%%%%%%%%% local definitions %%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%  Title page %%%%%%%%%%%%%%%%%%%%%%%%
\cmsNoteHeader{HIG-12-021} % This is over-written in the CMS environment: useful as preprint no. for export versions
...
sample corresponding to an integrated luminosity of 3.5~fb$^{-1}$ of
proton-proton collisions at $\sqrt{s}=8\,\text{Te\hspace{-.08em}V}$
and collected with the CMS detector at the CERN LHC in 2012.
...
angular spin correlations of the decay products.  No evidence for the
Higgs boson is found.  Using 8~TeV data, we exclude the Standard Model
Higgs boson in the mass range 260--390~GeV, to a confidence level of
95\%. When combined with 7~TeV data, the exclusion range expands to
240-450~GeV to 95\% confidence.  }
...
measurable contribution from vector boson fusion (VBF)~\cite{Ciccolini:2007jr,Ciccolini:2007ec,
...
The WW channel, 
where one W decays leptonically and allows for triggering the event, while the other decays
...
control the large W+jets background.
%%In this article we present an optimized search for the SM Higgs boson
%%decaying to two W bosons, one of which decays leptonically while the
...
results are reported in \cite{HIG-12-003}. The ATLAS experiment also
performed a search in this channel \cite{ATLAS:2011ae}. Here we
present the results from data corresponding to an integrated
luminosity of $3.50 \pm 0.18\fbinv$ collected with the CMS detector in
pp collisions at $\sqrt{s} = 8\TeV$ at the CERN LHC in 2012. These
results are presented both in isolation and in combination with our
results from 2011. No evidence for a Higgs boson is found, and limits
...
identifying muons up to $|\eta| < 2.1$. The detector is nearly hermetic, allowing for measurements
...
We use the \MADGRAPH~5~1.3.30~\cite{MADGRAPH} event
...
The {\sc powheg} generator has been used to produce Higgs signal events, and the showering has been
performed with \PYTHIA. For this analysis, samples with Higgs mass hypotheses ranging
from 170 to 600\GeVnn have been used. 
All simulated events are fully reconstructed using the standard CMS
software as used in real data.
...
on in-situ measurements using di-jet, $\gamma+{\rm jet}$, and Z+jet
...
According to simulation, in case of 2-jet events we select the 
right jet combination at a rate that varies from 68\% for 
$M_H = 200$~GeV to 88\% for $M_H = 600$~GeV.
For 3-jet events, the corresponding rates are 26\% and 84\%, respectively.
The events with wrong combination contribute as non-peaking 
...
%%Electrons (Muons) are therefore required to be isolated 
%%from hadronic activity in the detector by restricting
%%the sum of transverse momentum or energy in the tracker, 
%%ECAL, and HCAL, within a surrounding cone
%%of $\Delta R <0.3 (0.4)$, to be less than 5\% (10\%) of the measured
%%$\PT$ of the electron (muon). 
%%Before the isolation requirement is applied, pile-up energy is 
%%subtracted from the isolation cone.
...
hadronic W candidate, while the lepton plus $\MET$ system is
considered as the leptonic W candidate.

An accurate $\MET$ measurement is essential for reconstructing the
leptonic W, and from there to the full event kinematics of the WW
system. We use $\MET$ measured in the event using the full
particle-flow reconstruction. The $\MET$ resolution, measured as a
function of the sum $\ET$ ($\sum\ET$) of the
particle-flow~\cite{PFT-09-001} objects in the event, varies from 4\%
at $\sum\ET=60\GeVnn$ to 10\% at
$\sum\ET=350\GeVnn$~\cite{Chatrchyan:2011tn}.  We require $\MET>25$
(30)\GeVnn in the event in case of muon (electron) data.  In addition,
...
$\textnormal{W}\to\ell\nu$ decays, we then require that the transverse
mass~\cite{VBTF} of the leptonic W candidate be larger than 30\GeVnn.
...
the invariant mass of the di-jet system ($m_{jj}$) and of the
...
\begin{figure}[t!]
  ...
  \caption{The invariant mass of the di-jet system ($m_{jj}$) and of
   the four-body system
  ($m_{\ell{}\nu{}jj}$) after the physics object selection but 
  before optimization, for the case of muons 2-jets sample. 
...
\end{figure}
%%%%%%%%%%%%%%%%%
%%%%%%% Kalanand: June 18, 2012:  Descope fit based analysis for ICHEP.
% ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ----
...
%%%Figure~\ref{fig:backgroundShape} shows the four-body mass distribution of the observed events 
%%%after the preselections, overlapped to a fit of the chosen background-only shape, performed 
%%%outside the combination tool for illustrative purposes.
%%%
%%%
%%%\begin{figure}[htb]
%%% \begin{center}
...
%%%   multiplied by a factor 40 to make it more visible.)}
...
\begin{figure}[t!]
  ...
  Ref.~\cite{HIG-11-027}. 
  Also shown in addition are the likelihood discriminants for muon 
  plus 2 jet channel with selections optimized for Higgs mass 
  hypotheses 190\GeVnn and 500\GeVnn. 
  Error bars correspond to the statistical uncertainties. 
  }
  \label{fig:mvaPlots}
  \end{center}
...
The likelihood variable definition is optimized with dedicated simulation 
...
fit to the di-jet invariant mass distribution $m_{jj}$ of the two
...
shape is taken from simulation for Higgs mass hypotheses below
180\GeVnn. For higher masses the simulation doesn't have enough
...
Figure~\ref{fig:mjj_mH350} for muon plus 2 jet channel, with selections
optimized for Higgs mass hypotheses 200\GeVnn, 300\GeVnn, and
500\GeVnn. The uncertainties in the normalization obtained from the
fit are propagated to the limit calculation as systematics.
...
\begin{figure}[!t]
  \centering
  \subfigure[]{
  \includegraphics[width=0.32\textwidth]{plots/H200_Mjj_Muon_2jets_Stacked.pdf}
  }  
  \subfigure[]{ 
  ...
  \caption{\label{fig:mjj_mH350} The di-jet invariant mass 
  distribution with the fit projections, for the muon 2-jet 
  category, after selections optimized for the Higgs mass 
  hypotheses of (a) 200\GeVnn, (b) 300\GeVnn, and (c) 500\GeVnn. 
...
optimized for Higgs mass hypotheses 200\GeVnn, 300\GeVnn, and 500\GeVnn. 
...
\begin{figure}[!t]
  \centering
  \subfigure[]{
  \includegraphics[width=0.32\textwidth]{plots/H200_Mlvjj_Muon_2jets_WpJShape.pdf}
  }  
  \subfigure[]{ 
  ...
  signal region for the Higgs mass hypothesis of 200\GeVnn (a), 300\GeVnn (b), and 500\GeVnn (c),
...
shown in Figure~\ref{fig:mlvjj_mH350} for muon plus 2-jet channel with
selections optimized for Higgs mass hypotheses 200\GeVnn, 300\GeVnn,
and 500\GeVnn.

\begin{figure}[!t]
  \centering
  \subfigure[]{
  \includegraphics[width=0.32\textwidth]{plots/H200_Mlvjj_Muon_2jets_Stacked.pdf}
  }  
  \subfigure[]{ 
...
%%  The bottom row shows pull distribution computed as [(Data - Background)/ Background uncertainty].}
  ...
  muon 2-jet category, after selections optimized for the Higgs mass hypotheses of 
  200\GeVnn (a), 300\GeVnn (b), and 500\GeVnn (c).
  Error bars correspond to the statistical uncertainties. 
  ...
  Uncertainties in the signal stength and lineshape are listed in 
...
acceptance within our selections. Additional uncertainties are
implemented to account for the limitation of the narrow Higgs width
approximation in the Higgs simulation and the effect of interference
with the SM backgrounds, and for the uncertainty in scale caused by
exclusive binning in 2- and 3-jet categories~(\cite{CLS2}, Appendix
C).  The uncertainty due to luminosity is considered~\cite{lumiPAS},
as well as the one due to the measurement of the number of pile-up
events in data. The uncertainty on the reconstruction, identification
...
%%%%%%% ---------- Kalanand: June 18, 2012:  Descope fit based analysis for ICHEP.
%% considering the uncertainty in the
%% measurement of the jet energy scale for the fit-based limit extraction, and
%%%%%%% ---------- 
...
W mass in a \ttbar enriched sample between data and Monte Carlo, and
is listed in Table~\ref{tab:signalSyst}.
%%%%%%% ---------- Kalanand: June 18, 2012:  Descope fit based analysis for ICHEP.
...
We compute systematic uncertainty in the 
...
%%%This accounts for 7\% for $\mH < 500\GeVnn$, for 13\% for $\mH \ge 500\GeVnn$. 
...
%%%%%%% ---------- Kalanand: June 18, 2012:  Descope fit based analysis for ICHEP.
 \begin{table}[h!]
  ...
  \hline
   \end{tabular}
   \end{center}
   \caption{Sources of signal systematics considered in the analysis, 
...
the fit of the extrapolated background in the signal region.
...
plus background, when the signal strength is free to float or is 
fixed at a given value, is calculated. We present 95\% exclusion 
...
%%%On the left, the fit-based limit is shown, on the right the likelihood-based one.
%%%With the fit-based study we exclude the SM Higgs boson in the mass range $XXX-YYY\GeVnn$, while with
%%%the likelihood-based study we exclude the SM Higgs boson in the mass range $260-390\GeVnn$.
%%%As a final result, 
%%%the exclusion range provided by the analysis with the most sensitive expected limit has been chosen.
We exclude to 95\% confidence the SM Higgs boson in the mass range
$260-390\GeVnn$ from analyzing 8 TeV data.  Combining with 7 TeV
results~\cite{HIG-12-003}, we exclude the SM Higgs boson in
the mass range $240-450\GeVnn$, to 95\% confidence.
...
\section{Conclusions}
\label{sec:summary}
We have presented a search for the SM Higgs boson decaying into two W
bosons with semi-leptonic final state using proton-proton collision 
data corresponding to an
integrated luminosity of 3.5\fbinv at $\sqrt{s}=8\TeV$ recorded by the 
CMS experiment at the LHC.  
No evidence for the SM-like Higgs boson has been found.  
We have presented 95\% exclusion limits on its production cross section.  
Using 8~TeV data, we exclude the Standard
Model Higgs boson in the mass range $260-390$~GeV, to a confidence
level of 95\%. When combined with 7~TeV data, the exclusion range
expands to $240-450$~GeV, once again to 95\% confidence.
