...
TCanvas* plotting36GG( bool logScale=false ) 
{
 
  std::cout << "plotting mumu golden " << std::endl;
...
//       lineColor_["QCD"] = kYellow+4;
...

  {
    ...
    //      fname += "Zonenotiso_36pb";
    else
      fname += "Zmumu_20-120_36pb";
...
// 	    	    type.push_back("QCD");
  ...
  if( logScale ) cname += "Golden_log";
  else           cname += "Golden_lin";
  ...
  legend->AddEntry(dummyGraph,"   data","LEP");      
  if( logScale )
    {
...
//     latex.SetTextSize(txtSize3);
    //    latex.DrawLatex(txtX3,txtY3,"non isolated di-muon");
  }
