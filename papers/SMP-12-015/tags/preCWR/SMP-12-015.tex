...
% Do not edit by hand!
\RCS$Revision: 137692 $
\RCS$HeadURL: svn+ssh://svn.cern.ch/reps/tdr2/papers/SMP-12-015/tags/preCWR/SMP-12-015.tex $
\RCS$Id: SMP-12-015.tex 137692 2012-07-17 20:04:24Z kalanand $
%%%%%%%%%%%%% local definitions %%%%%%%%%%%%%%%%%%%%%
...
A measurement of the diboson production 
cross section is reported in events containing a leptonically decaying 
W boson and exactly two non-b jets, using 5.0~fb${}^{-1}$ data at 
$\sqrt{s} = 7$~TeV collected by the CMS detector at the LHC.
The measured value of the sum of WW and WZ cross sections is 
$66.47 \pm 8.40 \text{(stat)} \pm 7.51 \text{(syst)}$~pb, 
consistent with the standard model  prediction of $65.6 \pm 2.2$~pb. 
...
Overlapping minimum-bias events from other proton-proton 
collisions (pile-up) and the underlying event can contribute 
additional energy to the reconstructed jets. 
...
Only events with two jets above this threshold and within $|\eta|<2.6$ are 
selected for the analysis.
To reduce contamination from \ttbar background,
events are discarded if one or more jets pass b-quark jet identification 
criteria based on the existence of a secondary vertex within the jet~\cite{btag}. 
...
\begin{tabular} {lcl}
   ...
   Single top          &    MC        & (approx NNLO) 85 pb $\pm 5\%$~\cite{Kidonakis:2010tc,Kidonakis:2011wy,Kidonakis:2010ux}\\
...
\begin{table*}[htbp]
  ...
  The term $\mathcal{A}\varepsilon$ includes W and Z branching fractions.
}
  \label{tab:yields}
\begin{ruledtabular}
  \begin{tabular} {lll}
...
\hline
        Diboson (WW+WZ) &   1899 $\pm$ 389       &  783 $\pm$ 302   \\
        W plus jets     &   67384 $\pm$ 586      & 31644 $\pm$ 850  \\
        $\ttbar$        &   1662 $\pm$ 117       &  946 $\pm$  67   \\
        Single top      &   650 $\pm$ 33         &  308 $\pm$  17   \\
        Drell-Yan plus jets (Z+jets) &   3609 $\pm$ 155   &  1408 $\pm$ 64   \\
...
Theoretical uncertainty in acceptance, including  
uncertainties due to PDF and additional jet rejection, is 4\%. 
...
We compute WW+WZ cross section as 
$\sigma = N^{\text{Sig}}/ (\mathcal{A} \, \varepsilon \, {\mathcal{L}})$, 
...
The values of $N^{\text{Sig}}$ and $\mathcal{A} \times \varepsilon$ 
are given in Table~\ref{tab:yields} for each data category. 
Combining results of muon and electron categories, we obtain: 
$\sigma = 66.47 \pm 8.40 \text{(stat)} \pm 7.51 \text{(syst)}$~pb, 
...
diagrams have three boson vertex: WW$\gamma$ and WWZ couplings 
...
the other two parameters $\lambda_Z$ and $\Delta{\kappa_\gamma}$.  
We use dijet $p_T$ distribution, shown in Fig.~\ref{fig:Fig2}, 
...
Fig.~\ref{fig:Fig3}. At the 95\% confidence level (CL) 
...
has been performed using W plus two non-b jet events.
The measured value $66.47 \pm 8.40 \text{(stat)} \pm 7.51 \text{(syst)}$~pb 
