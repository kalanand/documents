...
% Do not edit by hand!
\RCS$Revision: 178938 $
\RCS$HeadURL: svn+ssh://svn.cern.ch/reps/tdr2/papers/HIG-12-034/tags/arxiv-0/HIG-12-034.tex $
\RCS$Id: HIG-12-034.tex 178938 2013-03-31 14:49:09Z alverson $
%%%%%%%%%%%%% local definitions %%%%%%%%%%%%%%%%%%%%%
...
\title{Search for a standard-model-like Higgs boson with a mass of up to 1\TeV at the LHC}
...
pdftitle={Search for a standard-model-like Higgs boson with a mass of up to 1 TeV at the LHC}, %
pdfsubject={CMS},%
...
$125\GeV$~\cite{ATLASobservation125,CMSobservation125,CMSlongpaper}. The ATLAS experiment
excluded at 95\% CL the mass ranges
111--122 and 131--559\GeV~\cite{ATLASobservation125}.
The analysis presented in this paper uses this data set to extend the CMS searches up to 1\TeV.
...
The $\PH \to \WW (\ZZ)$ signals are simulated using the next-to-leading order (NLO) package
...
the effects on the lineshape due to interference between Higgs boson signal and the  $\Pg\Pg \to \WW\ (\ZZ)$
background are included~\cite{Passarino:2012ri,Campbell:2011cu}. The theoretical uncertainties on the lineshape due to missing
...
\textsc{gg2zz}~\cite{Binoth:2008pr}. Other diboson processes ($\PW\cPZ$, $\cPZ\gamma^{(*)}$, $\PW\gamma^{(*)}$) and $\cPZ+$jet are generated with \PYTHIA6.424 and \MADGRAPH. The $\ttbar$ and $\tw$ events are generated at NLO with \POWHEG.
For all samples
...
The efficiencies for selecting electrons in the ECAL barrel (endcaps) varies from around
...
For the $\PW\PW \to \ell\nu\ell\nu$ and
$\cPZ\cPZ \to 2\ell 2\ell'$ channels the full possible mass range is 110--1000\GeV, but in this paper
both analyses are restricted to the masses above 145\GeV. The
$\cPZ\cPZ \to 2\ell 2\cPq$ analysis uses only 7\TeV data.
}
\label{tab:channels}
...
a third lepton passing the identification and isolation requirements reduces both $\WZ$ and $\wgamma^*$ backgrounds.
...
azimuthal angle between the $\vecPtell$ and the jet are also used. The BDT classifier distributions for $\mH=500\GeV$ are shown in Fig.~\ref{fig:hww2l2n_bdt_500}(\cmsRight) for the 0-jet different-flavor category. BDT training is performed using $\Hww$ as signal and non-resonant $\WW$ as background. The binned observed BDT distributions are fitted to the sum of templates for the signal and background BDT distributions.
...
\centering
   ...
   BDT selection level in the 0-jet bin different-flavor category.}
...
dominated by the theoretical uncertainty associated with missing higher-order QCD corrections and PDF uncertainties. The total uncertainty on the background estimation in the $\Hww$ signal region is about 15\% and is dominated by the
...
\breakhere$\abs{\Delta\phi_{\textrm{leading jet,\MET}}} > 0.8\,(0.4)$ are imposed for electrons (muons). These criteria reduce the QCD multijet background, for which in many cases the $\MET$ is generated by a mismeasurement of a jet energy.\par}

To improve the $m_{\WW}$ resolution, both $\PW$ candidates are constrained in a kinematic fit to the $\PW$-boson
mass to within its known width, with the longitudinal component of the neutrino momentum, $p_z$ unknown. The ambiguity is resolved by taking the solution that yields the smaller $\abs{p_z}$ value for the neutrino.
...
mass hypothesis and
event category are binned and fed as input to the limit-setting procedure.
...
scales and the PDF set, following the PDF4LHC recommendations~\cite{Alekhin:2011sk,Botje:2011sn,Lai:2010vv,Martin:2009iq,Ball:2011mu}. The uncertainties associated
...
lepton selections. To estimate the number of expected background events in the signal region,
...
$\PH \to \ZZ \to 2\ell 2\ell '$ channel. For the $\cPZ$+X background, the estimations are based on data. The $4\Pe$, $4\Pgm$, and $2\Pe2\Pgm$ yields are
...
combination of the $4\Pe$, $4\Pgm$, and $2\Pe2\Pgm$
...
This channel has the largest branching fraction of all $\PH \to \ZZ$ channels, but also a large background
...
of the dilepton system is required to be greater than $55\GeV$. Jets are required to have $\pt>30\GeV$ and $\abs{\eta}<5$. The presence of large missing transverse energy in the
...
estimate. This treatment considers only $\PH \to \cPZ\cPZ$ channel as signal and is combined
...
non-resonant background is evaluated to be 25\%. The expected signal and background, and the observed event yields, for
...
the result of combination is determined by $\PW\PW\to\ell\nu\ell\nu$ channel.
...
be more and more effective in $2\ell 2\nu$ channel.
Finally at $\mH>600\GeV$ the sensitivity of the combination is fully determined by the two $\cPZ\cPZ$
decay channels.
